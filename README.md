...
7. Развернут Logstash. Настроена очередь DLQ.
8. Logstash по-прежнему принимает "сырые" данные от filebeat, поэтому также попадает под действие политики ILM
9. Развернут Vector и парсит логи nginx с помощью VRL
